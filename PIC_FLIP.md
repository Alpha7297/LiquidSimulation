...
$$
实现时不能在求解压强前直接把固体边界相邻face的速度全部置0，否则右端项会丢失撞墙的法向通量。当前实现保留预测速度中的固体法向face速度参与散度计算，并在Gauss-Seidel更新中把固体邻居的Neumann贡献加入 `solid_term`。投影完成后，再把固体法向速度置0。

同时，空气区域的速度不应简单置0。否则自由表面附近的G2P采样会混入大量零速度，表现为很强的黏滞和飞溅被抹掉。当前实现会在投影后把液体附近的有效face速度向空气区域外推一层，再用于G2P采样。外推层数不能太多，否则上方自由表面的速度会在空气中传播过远，反过来牵扯下方粒子。
