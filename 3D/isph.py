...
pred_vel=ti.Vector.field(3,dtype=ti.f32,shape=PARTICLE_N)
...
@ti.func
def poisson_coeff(i:int,nb:int,r:ti.f32,grad:vec3)->ti.f32:
    rij=pos[i]-pos[nb]
...
def init_particles():
    ...
    for idx in range(FLUID_N):
        ...
        pred_vel[idx]=ti.Vector([0.0,0.0,0.0])
    ...
    for y,z,l in ti.ndrange(SIDE_ROWS,CONTAINER_N,SIDE_LAYERS):
        ...
        pred_vel[left]=ti.Vector([0.0,0.0,0.0])
        pred_vel[right]=ti.Vector([0.0,0.0,0.0])
    ...
    for y,x,l in ti.ndrange(SIDE_ROWS,CONTAINER_N,SIDE_LAYERS):
        ...
        pred_vel[front]=ti.Vector([0.0,0.0,0.0])
        pred_vel[back]=ti.Vector([0.0,0.0,0.0])
        ...
        pressure[front]=0.0
        pressure[back]=0.0
        tmp_pressure[front]=0.0
        tmp_pressure[back]=0.0
        pressure_diag[front]=1.0
        pressure_diag[back]=1.0
        pressure_rhs[front]=0.0
        pressure_rhs[back]=0.0
        mass[front]=MASS
        mass[back]=MASS
        edge[front]=1
        edge[back]=1
...
def predict_velocity():
    ...
    for i in range(PARTICLE_N):
        if edge[i]==0:
            ...
            pred_vel[i]=ti.Vector([0.0,0.0,0.0])
            pressure[i]=0.0
...
def render(window,scene,camera):
    ...
    scene.particles(fluid_pos,radius=PART_RADIUS,color=(0.15,0.58,0.96))
...
def render_gui(window):
    gui=window.get_gui()
    gui.begin("3D ISPH",0.02,0.02,0.27,0.26)
...
def main():
    init()
    window=ti.ui.Window("3D ISPH",(1100,900))
    scene=ti.ui.Scene()
    camera=ti.ui.Camera()
    init_camera(camera)
